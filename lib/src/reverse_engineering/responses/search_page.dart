...
class SearchPage {
  ...
  String get apiKey => _apiKey ??= _apiKeyExp
      .firstMatch(_root.querySelectorAll('script').firstWhere((e) => e.text.contains('INNERTUBE_API_KEY')).text)
      .group(1);
  ...
  _InitialData get initialData {
    ...
    var scriptTag = _extractJson(
        _root
            ...
            .map((e) => e.text)
            .toList()
            .firstWhere((e) => e.contains('window["ytInitialData"] ='), orElse: () => null),
    ...
    scriptTag ??= _extractJson(
        _root
            ...
            .map((e) => e.text)
            .toList()
            .firstWhere((e) => e.contains('var ytInitialData ='), orElse: () => '{}'),
        'var ytInitialData =');
  ...
  ///
  SearchPage(this._root, this.queryString, [_InitialData initialData, this._apiKey]) : _initialData = initialData;
  ...
  Future<SearchPage> nextPage(YoutubeHttpClient httpClient) async {
    ...
    return get(httpClient, queryString, token: initialData.continuationToken, key: apiKey);
  }

  ///
  static Future<SearchPage> get(YoutubeHttpClient httpClient, String queryString, {String token, String key}) {
    if (token != null) {
      ...
      return retry(() async {
        ...
        return SearchPage(null, queryString, _InitialData(SearchPageId.fromJson(json.decode(raw.body))), key);
      });
      // Ask for next page,
...
class _InitialData {
  ...
  List<PurpleContent> getContentContext() {
    if (root.contents != null) {
      return root.contents.twoColumnSearchResultsRenderer.primaryContents.sectionListRenderer.contents.first
          .itemSectionRenderer.contents;
    }
    if (root.onResponseReceivedCommands != null) {
      return root.onResponseReceivedCommands.first.appendContinuationItemsAction.continuationItems[0]
          .itemSectionRenderer.contents;
    }
    return null;
  }
  ...
  String _getContinuationToken() {
    if (root.contents != null) {
      var contents = root.contents.twoColumnSearchResultsRenderer.primaryContents.sectionListRenderer.contents;

      if (contents.length <= 1) {
        return null;
      }
      return contents[1].continuationItemRenderer.continuationEndpoint.continuationCommand.token;
    }
    if (root.onResponseReceivedCommands != null) {
      return root.onResponseReceivedCommands.first.appendContinuationItemsAction.continuationItems[1]
  ...
  List<BaseSearchContent> get searchContent =>
      _searchContent ??= getContentContext().map(_parseContent).where((e) => e != null).toList();
  ...
  BaseSearchContent _parseContent(PurpleContent content) {
    ...
    if (content.videoRenderer != null) {
      ...
      return SearchVideo(
          ...
          null,
          int.parse(renderer.lengthText?.simpleText ?? ''),
          null);
    }
    if (content.radioRenderer != null) {
      ...
      return SearchPlaylist(PlaylistId(renderer.playlistId), renderer.title.simpleText,
          int.parse(_parseRuns(renderer.videoCountText.runs).stripNonDigits().nullIfWhitespace ?? 0));
    }
